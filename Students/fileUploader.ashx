...
Public Class fileUploader : Implements IHttpHandler
    ...
    Public Sub ProcessRequest(ByVal context As HttpContext) Implements IHttpHandler.ProcessRequest
        ...
        Try
            Dim dirFullPath As String = HttpContext.Current.Server.MapPath("~/UploadedDocuments/")
            Dim files As String()
            ...
            For Each s As String In context.Request.Files
                ...
                '    str_image = Convert.ToString("SHT_PHOTO_" + numFiles.ToString()) & fileExtension
                '    pathToSave_100 = HttpContext.Current.Server.MapPath("~/StudentsPassport/") & str_image
                '    file.SaveAs(pathToSave_100)
                'End If
                
                If Not String.IsNullOrEmpty(fileName) Then
                    fileExtension = Path.GetExtension(fileName)
                    str_image = "SHT_PHOTO_" + numFiles.ToString() + fileExtension
                    Dim pathToSave As String = HttpContext.Current.Server.MapPath("~/UploadedDocuments/") + str_image
            ...
            context.Response.Write("~/UploadedDocuments/" & str_image)
